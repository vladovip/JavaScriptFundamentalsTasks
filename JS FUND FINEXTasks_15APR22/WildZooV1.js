  ...
  while (textLine != "EndDay") {
   ...
   //  console.log(currentCommand , animalName, food , area );
    ...
    let area = animalInfoLine[2];
      ...
      if (animalAreaObj.hasOwnProperty(area) == false) {
        animalAreaObj[area] = [];
        animalAreaObj[area].push(animalName);
      //   console.log(animalAreaObj[area]);
      } else if (animalAreaObj.hasOwnProperty(area) == true) {
        if (animalAreaObj[area].includes(animalName) == false) {
          animalAreaObj[area].push(animalName);
         //  console.log(animalAreaObj[area]);
        }
      }
      ...
      if (animalRecordsObj.hasOwnProperty(animalName) == true) {
        if (animalRecordsObj[animalName].neededFood > food) {
          animalRecordsObj[animalName].neededFood -= food;
        } else if (animalRecordsObj[animalName].neededFood <= food ) {
          console.log(`${animalName} was successfully fed`);
          delete animalRecordsObj[animalName];
          animalAreaObj[area].join("").replace(animalName,"").split(", ");  
        }
      }
    } 
   ...
   for (let areas in animalAreaObj) {
   console.log(`${areas} -> ${animalAreaObj[areas].length}`);
   }
