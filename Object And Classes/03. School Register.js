        ...
        if (!mainObj.hasOwnProperty(gradeOfFirstStudent.toString())) {
            ...
            for (let j = i + 1; j < studentsWhoPass.length; j++) {
                ...
                if (gradeOfSecondStudent === gradeOfFirstStudent) {
                    if (mainObj.hasOwnProperty(gradeOfSecondStudent.toString())) {
            ...
            mainObj[gradeOfFirstStudent]['Average annual grade from last year'] = avgScore / (names.length);
        }
    } for (let element of Object.entries(mainObj)) {
        let tokens = Object.entries(element[1]);
        console.log(`${Number(element[0]) + 1} Grade\n${tokens[0][0]}: ${tokens[0][1].join(', ')}\n${tokens[1][0]}: ${(tokens[1][1]).toFixed(2)}\n`);
    }
}

studentsRegister(["Student name: Mark, Grade: 8, Graduated with an average score: 4.75",
